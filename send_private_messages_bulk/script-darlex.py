...
def get_dialog_status(driver, name):
    ...
    names = [u'{} {}'.format(i.get('firstName', ''), i.get('lastName', '')) for i in parts]
...
def process_messages(fname, text_fname):
    ...
    for name in open(fname):
        ...
        try:
            ...
            #    print 'canceled, dialog exist', name.encode('utf8')

            #    continue

            sent = send_msg(driver, name, text.format(name=name))

            if sent:

                print number, 'sent', name.encode('utf8')
            ...
            print 'failed, error={}'.format(e.message), name.encode('utf8')
